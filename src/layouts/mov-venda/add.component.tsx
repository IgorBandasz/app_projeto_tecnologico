...
import { movVendaInsertSchema } from './schemas';
...
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [model, setModel] = React.useState<IMovimentacaoPneu>({ DataString: dataAtual(), HoraString: horaAtual(), DataVendaString: dataAtual(), ValorVenda: 0,
                                            TipoMov: 4, SulcoAtual: param.pneu?.SulcoAtual, Pneu: param.pneu} as IMovimentacaoPneu);
        ...
        if (result.status === 201) {
          ...
          if (addImagens){
            ...
            if (resultMov.data.Data){
              ...
              setIsLoading(false);
              navigation.navigate('MovVendaEdit', { 
                voltas: voltas,
            ...
            await param.onGoBack();
            Alert.alert('Sucesso', `Pneu enviado para a venda com sucesso.`, [
              { text: 'Ok', onPress: () => { navigation.pop(param.voltas) }, }],
              { cancelable: false },
            );
          }
  ...
  const formik = useFormik({
    ...
    validationSchema: movVendaInsertSchema,
