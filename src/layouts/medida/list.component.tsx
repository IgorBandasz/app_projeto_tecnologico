...
import { MedidaList } from './medida-list.component';
import { IMedida } from '../../model/medida.model';
import MedidaService from '../../services/api-medida-service';

interface Params { 
  onSelect: (Item: IMedida) => void;
}
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<IMedida[]>([]);
  ...
  async function handleSelect(item: IMedida) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
