...
export interface MedidaListProps extends Omit<ListProps, 'renderItem'> {
  data: IMedida[];
  onHandleView: (item:  IMedida) => void;
  onHandleFindMore: (distance: number) => void;
}

export type LayoutListElement = React.ReactElement<MedidaListProps>;

export const MedidaList = (props: MedidaListProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { contentContainerStyle, onHandleView, ...listProps } = props;

  const renderItem = (info: ListRenderItemInfo<IMedida>): CardElement => (
