...
export interface MedidaCardProps extends Omit<CardProps, 'renderItem'> {
  data: IMedida;
  onFindPress: (item?: IMedida) => void;
  caption?: string;
}

export const MedidaCard = (props: MedidaCardProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { onFindPress } = props;
  const swipeableRef = useRef(null);

  function clickFind(item: IMedida) {
    swipeableRef?.current?.close();
    onFindPress(item)
  }

  async function onFind(item: IMedida) {
    if(!item)
      props.onFindPress()
  }
  ...
  return (
    <>
      <Text style={styles.textLabel} appearance='hint' category='label' > Medida </Text>
        ...
        <Swippeable
          ...
          <RectButton
            ...
            <View>
              ...
              <Text category='s2' >
                {props.data ? `Nome: ${props.data?.Nome}` : 'Sem Medida Selecionado'}
              </Text>
            </View>
