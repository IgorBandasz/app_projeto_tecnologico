...
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<IBem[]>([]);
  ...
  async function handleFind(more: boolean) {
    try {
      ...
      const frota     = pesquisaIndex.row === 0 ? pesquisa : '';
      const placa     = pesquisaIndex.row === 1 ? pesquisa : '';
      const id        = pesquisaIndex.row === 2 ? pesquisa : '';
      const descricao = pesquisaIndex.row === 3 ? pesquisa : '';
      
      const service = new BemService();
      const result = await service.getAll(page, id, descricao, frota, placa);
      
      if (result.status == 200){
        if (page > 1) {
          setList(oldValue => [...oldValue, ...result.data])
        } else {
          setList(result.data);
        }
      }else{
  ...
  async function handleSelect(item: IBem) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
