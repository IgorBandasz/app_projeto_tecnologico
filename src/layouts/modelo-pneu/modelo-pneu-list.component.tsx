...
export interface ModeloPneuListProps extends Omit<ListProps, 'renderItem'> {
  data: IModeloPneu[];
  onHandleView: (item:  IModeloPneu) => void;
  onHandleFindMore: (distance: number) => void;
}

export type LayoutListElement = React.ReactElement<ModeloPneuListProps>;

export const ModeloPneuList = (props: ModeloPneuListProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { contentContainerStyle, onHandleView, ...listProps } = props;

  const renderItem = (info: ListRenderItemInfo<IModeloPneu>): CardElement => (
