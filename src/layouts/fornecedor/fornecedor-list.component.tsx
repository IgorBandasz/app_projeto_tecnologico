...
export interface FornecedorListProps extends Omit<ListProps, 'renderItem'> {
  data: IFornecedor[];
  onHandleView: (item:  IFornecedor) => void;
  onHandleFindMore: (distance: number) => void;
}

export type LayoutListElement = React.ReactElement<FornecedorListProps>;

export const FornecedorList = (props: FornecedorListProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { contentContainerStyle, onHandleView, ...listProps } = props;

  const renderItem = (info: ListRenderItemInfo<IFornecedor>): CardElement => (
