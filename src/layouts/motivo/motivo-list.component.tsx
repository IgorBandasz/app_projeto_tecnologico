...
export interface MotivoListProps extends Omit<ListProps, 'renderItem'> {
  data: IMotivo[];
  onHandleView: (item:  IMotivo) => void;
  onHandleFindMore: (distance: number) => void;
}

export type LayoutListElement = React.ReactElement<MotivoListProps>;

export const MotivoList = (props: MotivoListProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { contentContainerStyle, onHandleView, ...listProps } = props;

  const renderItem = (info: ListRenderItemInfo<IMotivo>): CardElement => (
