...
export interface MotivoCardProps extends Omit<CardProps, 'renderItem'> {
  data: IMotivo;
  onFindPress: (item?: IMotivo) => void;
  caption?: string;
}

export const MotivoCard = (props: MotivoCardProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { onFindPress } = props;
  const swipeableRef = useRef(null);

  function clickFind(item: IMotivo) {
    swipeableRef?.current?.close();
    onFindPress(item)
  }

  async function onFind(item: IMotivo) {
    if(!item)
      props.onFindPress()
  }
  ...
  return (
    <>
      <Text style={styles.textLabel} appearance='hint' category='label' > Motivo </Text>
        ...
        <Swippeable
          ...
          <RectButton
            ...
            <View>
              ...
              <Text category='s2' >
                {props.data ? `Nome: ${props.data?.Nome}` : 'Sem Motivo Selecionado'}
              </Text>
            </View>
