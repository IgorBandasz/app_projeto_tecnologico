...
import { MotivoList } from './motivo-list.component';
import { IMotivo } from '../../model/motivo.model';
import MotivoService from '../../services/api-motivo-service';

interface Params { 
  onSelect: (Item: IMotivo) => void;
}
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<IMotivo[]>([]);
  ...
  async function handleFind(more: boolean) {
    try {
      ...
      const service = new MotivoService();
  ...
  async function handleSelect(item: IMotivo) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
