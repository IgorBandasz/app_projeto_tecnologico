...
import { IConsertoPneuServico } from '../../../model/conserto-pneu.model';
...
export interface ConsertoServicoListProps extends Omit<ListProps, 'renderItem'> {
  data: IConsertoPneuServico[];
  onAddPress?: () => void;
  onViewPress?: (item: IConsertoPneuServico) => void;
  onDeletePress?: (item: IConsertoPneuServico) => void;
}

export type LayoutListElement = React.ReactElement<ConsertoServicoListProps>;

export const ConsertoServicoList = (props: ConsertoServicoListProps): ListElement => {
  ...
  function clickView(index: number, item: IConsertoPneuServico) {    
    prevOpenedRow = row[index];
    prevOpenedRow.close();
    props.onViewPress(item);
  }

  function clickDelete(index: number, item: IConsertoPneuServico) {    
    prevOpenedRow = row[index];
    prevOpenedRow.close();
    deletePressButton(item)
  }

  async function deletePressButton(servico: IConsertoPneuServico) {
    setIsLoading(true);
    props.onDeletePress(servico);
    setIsLoading(false);
  }

  const renderItem = (info: ListRenderItemInfo<IConsertoPneuServico>): CardElement => (
    <Card>
