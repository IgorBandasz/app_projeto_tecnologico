...
import { recapagemPneuUpdateSchema } from './schemas';
...
import { RecapagemServicoList } from './servico/servico-list.component';
...
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [model, setModel] = React.useState<IRecapagemPneu>(param.data);
  ...
  async function handleAddServico() {
    navigation.navigate('RecapagemPneuServicoAdd', { pneu: formik.values.Pneu ,onHandleSave: onAddServicoButtonPress });
  }

  async function handleViewServico(item: IRecapagemPneuServico) {
    navigation.navigate('RecapagemPneuServicoView', { idRecapagem: id, data: item });
  }

  async function onAddServicoButtonPress(item: IRecapagemPneuServico) {
    if(isLoading == false){
      try {
        setIsLoading(true);
  ...
  async function onDeleteServicoButtonPress(item: IRecapagemPneuServico) {
    try {
      setIsLoading(true);
      ...
      try {
        ...
        if (result.status === 200){
          setModel({...result.data.Data,
            ...
            DataPrevEntregaString: dateDBToStr(result.data.Data.DataPrevEntrega)} as IRecapagemPneu);
        } else {
          Alert.alert('Aviso', result.data?.Message)
        }
  ...
  async function atualizaTotais():Promise<boolean> {
    try {
      ...
      if (result.status === 200){
        const recapagem = {...result.data.Data,
                        DataString: dateDBToStr(result.data.Data.Data),
                        HoraString: timeDBToStr(result.data.Data.Hora),
                        DataEnvioString: dateDBToStr(result.data.Data.DataEnvio),
                        DataPrevEntregaString: dateDBToStr(result.data.Data.DataPrevEntrega),
                        Servicos: formik.values.Servicos} as IRecapagemPneu;
  ...
  const handleSave = async (values: IRecapagemPneu): Promise<void> => {
    if(isLoading == false){
      try {
        ...
        if (result.status === 200) {
          await param.onGoBack();
          Alert.alert('Sucesso', `Recapagem alterada com sucesso. Id: ${result.data.Id}`, [
  ...
  const formik = useFormik({
    ...
    validationSchema: recapagemPneuUpdateSchema,
          ...
          <Tab title='Dados' icon={DadosIcon}>
            <>
              ...
              <SafeAreaLayout
                ...
                <DesenhoCard
                  data={model.DesenhoSugestao}
                  onFindPress={null} />  

                <FornecedorCard
                  data={model.Recapadora}
                  onFindPress={null} />  
