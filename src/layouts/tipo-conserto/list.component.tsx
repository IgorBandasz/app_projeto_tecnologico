...
import { TipoConsertoList } from './tipo-conserto-list.component';
import { ITipoConserto } from '../../model/tipo-conserto.model';
import TipoConsertoService from '../../services/api-tipo-conserto-service';

interface Params { 
  onSelect: (Item: ITipoConserto) => void;
}
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<ITipoConserto[]>([]);
  ...
  async function handleFind(more: boolean) {
    try {
      ...
      const service = new TipoConsertoService();
  ...
  async function handleSelect(item: ITipoConserto) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
