...
export interface TipoConsertoListProps extends Omit<ListProps, 'renderItem'> {
  data: ITipoConserto[];
  onHandleView: (item:  ITipoConserto) => void;
  onHandleFindMore: (distance: number) => void;
}

export type LayoutListElement = React.ReactElement<TipoConsertoListProps>;

export const TipoConsertoList = (props: TipoConsertoListProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { contentContainerStyle, onHandleView, ...listProps } = props;

  const renderItem = (info: ListRenderItemInfo<ITipoConserto>): CardElement => (
