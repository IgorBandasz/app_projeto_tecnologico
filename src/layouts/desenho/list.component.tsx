...
import { DesenhoList } from './desenho-list.component';
import { IDesenho } from '../../model/desenho.model';
import DesenhoService from '../../services/api-desenho-service';

interface Params { 
  onSelect: (Item: IDesenho) => void;
}
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<IDesenho[]>([]);
  ...
  async function handleFind(more: boolean) {
    try {
      ...
      const service = new DesenhoService();
  ...
  async function handleSelect(item: IDesenho) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
