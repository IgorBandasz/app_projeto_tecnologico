...
export interface FabricantePneuCardProps extends Omit<CardProps, 'renderItem'> {
  data: IFabricantePneu;
  onFindPress: (item?: IFabricantePneu) => void;
  caption?: string;
}

export const FabricantePneuCard = (props: FabricantePneuCardProps): ListElement => {

  const styles = useStyleSheet(themedStyles);
  const { onFindPress } = props;
  const swipeableRef = useRef(null);

  function clickFind(item: IFabricantePneu) {
    swipeableRef?.current?.close();
    onFindPress(item)
  }

  async function onFind(item: IFabricantePneu) {
    if(!item)
      props.onFindPress()
  }
  ...
  return (
    <>
      <Text style={styles.textLabel} appearance='hint' category='label' > Fabricante de Pneu </Text>
        ...
        <Swippeable
          ...
          <RectButton
            ...
            <View>
              ...
              <Text category='s2' >
                {props.data ? `Nome: ${props.data?.Nome}` : 'Sem Fabricante de Pneu Selecionado'}
              </Text>
            </View>
