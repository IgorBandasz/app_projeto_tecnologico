...
import { FabricantePneuList } from './fabricante-pneu-list.component';
import { IFabricantePneu } from '../../model/fabricante-pneu.model';
import FabricantePneuService from '../../services/api-fabricante-service';

interface Params { 
  onSelect: (Item: IFabricantePneu) => void;
}
export default ({ navigation, route }): React.ReactElement => {
  ...
  const [list, setList] = useState<IFabricantePneu[]>([]);
  ...
  async function handleFind(more: boolean) {
    try {
      ...
      const service = new FabricantePneuService();
  ...
  async function handleSelect(item: IFabricantePneu) {
    if(isLoading == false){
      setIsLoading(true);
      if (param?.onSelect) {
        param.onSelect(item);
